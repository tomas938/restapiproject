			...
			<div class="search">
				...
				>
					...
					<div class="filter__links" :class="{ active: clicked, light: theme }">
						<ul>
							<li
								@click="getRegionName('All')"
								class="filter-link"
								:class="{ lighttext: theme }"
							>
								All
							</li>
							<li
								@click="getRegionName('Africa')"
								class="filter-link"
								:class="{ lighttext: theme }"
							>
								Africa
			...
			<ul class="grid">
				...
				>
					...
					<div class="text" :class="{ white: theme }">
						...
						<h4 class="country-region" :class="{ white: theme }">
...
export default {
	...
	methods: {
		...
		getCoutryName() {
			...
			this.coutnryName.forEach((name) => {
				if (
					name.innerText
						.toLowerCase()
						.includes(this.inputedcountry.toLowerCase())
				) {
		...
		getRegionName(region) {
			this.countryRegion = document.querySelectorAll(".country-region");

			this.countryRegion.forEach((countryregion) => {
				if (countryregion.innerText.includes(region) || region === "All") {
					countryregion.parentElement.parentElement.style.display = "grid";
				} else {
					countryregion.parentElement.parentElement.style.display = "none";
				}
			});
			// this.filterLinks.forEach((link) => {
			// 	this.val = link.innerText;
			// 	console.log(this.val);
			// 	this.countryRegion.forEach((region) => {
			// 		if (region.innerText.includes(this.val) || this.val === "All") {
			...
			// 	});
			// });
			// 	this.coutnryRegion.forEach((region) => {
			// 		if (region.innerText.includes(this.val) || this.val === "All") {
			// 			region.parentElement.parentElement.style.display = "grid";
			// 		} else {
			// 			region.parentElement.parentElement.style.display = "none";
